...
It provides functions to tidy the _sumario_ and retrieve the individual ".pdf" of each publication and summaries. 
...
Note at most there is one _sumario_ per day, so 365 is the max, but usually none is published on Sundays. 
...
summary number, section, department, epigraph, brief text of the publication, 
publication's number of pages of the pdf.

The easiest way to get a _sumario_ is using [`retrieve_sumario()`]:
...
Check it [here](`r query_pdf("2017", "10", "02", cve)`).
